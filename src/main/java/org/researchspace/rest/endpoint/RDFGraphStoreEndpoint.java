 ...
 * Create the graph with {IRI} and message body. Content-type must be specified
 * in the header. If {IRI} is not specified statements will be added to the
 * default/null graph or to the graphs as specified in the serialization format
 * (i.e. if format is e.g. TRIG/NT/JSON-LD).</li>
 * <li>PUT /rdf-graph-store?graph={IRI} <br/>
 * Updates the graph with {IRI} and message body. Graph must exist. Content-type
 * must be specified in the header.</li>
...
public class RDFGraphStoreEndpoint {
    ...
    public Response createGraph(@QueryParam("graph") Optional<IRI> uri,
            @QueryParam("keepSourceGraphs") boolean keepSourceGraphs,
            @QueryParam("repository") Optional<String> repository, InputStream in) throws URISyntaxException {
        ...
        try (RepositoryConnection con = getRepository(repository).getConnection()) {
            ...
            if (cause instanceof UpdateExecutionException && cause.getMessage().equals("Broken pipe")) {
                ...
                msgBuilder.append(
                        "To increase the maximum body size you may want to re-start the platform while setting ");
                msgBuilder.append(
                        "\"-Dorg.eclipse.jetty.server.Request.maxFormContentSize=<MAX_SUPPORTED_NUM_BYTES>\" as a JVM parameter.");

                logger.error("Failed to create GRAPH \"{}\" :", graphUri, msgBuilder);

                // and return a payload too large exception, indicating that the file size is
                // too large
                return Response.status(413 /* payload too large */).type("text/plain").entity(msgBuilder.toString())
                        .build();
    ...
    public Response deleteGraph(@QueryParam("graph") IRI uri, @QueryParam("repository") Optional<String> repository)
    ...
    public Response updateGraph(@QueryParam("graph") Optional<IRI> uri,
            @QueryParam("repository") Optional<String> repository, InputStream in) throws URISyntaxException {
    ...
    /**
     * Generates random GRAPH identifiers i.e. according to the spec, the API should
     * create identifiers and return in the location header, if not explicitly
     * specified as parameter by the client.
     *
     * @return
     */
    private IRI getAutoGeneratedGraphUri() {
        ...
        String platformBaseIri = StringUtils.isEmpty(platformBaseIriConfig) ? uriInfo.getBaseUri().toString()
    ...
    private Repository getRepository(Optional<String> repository) {
        return repository.isPresent() ? repository.map(rep -> repositoryManager.getRepository(rep)).get()
