...
export class SemanticTable extends Component<SemanticTableProps, TableState> {
  ...
  private handleFilterChange(
    ...
    if (typeof filter === 'string') {
      this.setState((state) => {
        let newFilters = state.filters.filter((f) => f.variableName !== variableName);
        if (filter !== '') {
          newFilters = newFilters.concat({
            filter: { filterType: 'text', variableType: variableType, date: { from: '', to: '' }, text: filter },
      ...
      this.setState((state) => {
        ...
        if (filter.filter.date.from !== '' || filter.filter.date.to !== '') {
          newFilters = newFilters.concat({
            filter: {
              date: { from: filter.filter.date.from, to: filter.filter.date.to },
              text: '',
              filterType: 'date',
      ...
      if (this.state.filters) {
        this.state.filters.forEach((f) => {
          if (f.filter.filterType === 'text') {
            ...
            if (f.filter.variableType === 'literal') {
              ...
              const filter: Pattern = {
                ...
                expression: {
                  type: 'operation',
                  operator: '=',
                  args: [`?${f.variableName}`, searchVariable],
                },
              };
              parsedQuery.where.push(searchPattern);
              parsedQuery.where.push(optional);
              parsedQuery.where.push(filter);
            } else {
              parsedQuery.where.push(searchPattern, triples);
            }
          } else if (f.filter.filterType === 'date') {
            const labelVariable = `?${f.variableName}_dateTime`;

            const filterDateFrom: Pattern = {
              type: 'filter',
              expression: {
                type: 'operation',
                operator: '>=',
                args: [
                  labelVariable,
                  {
                    ...
                    args: [`"${f.filter.date.from}T00:00:00Z"`],
                  },
                ],
              },
            };
            const filterDateTo: Pattern = {
              type: 'filter',
              expression: {
                type: 'operation',
                operator: '<=',
            ...
            if (f.filter.date.from) parsedQuery.where.push(filterDateFrom);
            if (f.filter.date.to) parsedQuery.where.push(filterDateTo);
          }
        });
      }
    }
